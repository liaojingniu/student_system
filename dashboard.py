...
# --- 3. 缓存并初始化 Supabase 客户端 ---
@st.cache_resource
def init_supabase():
    # 直接从 Streamlit 云端的 Secrets 安全读取
...
# --- 4. 优化数据加载（将 ttl 延长至 600 秒/10分钟，大幅提升打开速度）---
...
def load_data():
    ...
    if not df_s.empty and not df_g.empty:
        df_merged = pd.merge(
            df_g, df_s, left_on="student_id", right_on="student_id", how="left"
        )
...
# 增加“手动刷新数据”按钮，方便数据库数据更改后实时重置缓存
if st.sidebar.button("🔄 刷新最新数据"):
    st.cache_data.clear()
    st.rerun()

schools = (
    ["全部学校"] + list(df["school_name"].dropna().unique())
...
filtered_df = (
    df if selected_school == "全部学校" else df[df["school_name"] == selected_school]
)

st.sidebar.markdown("---")
st.sidebar.success(
    "📄 提示：按 **`Ctrl + P`** 可导出无侧边栏的 2页 A4 PDF 报告！"
)
...
# 换成图表 1：学科平均分折线图
st.markdown(
    "<div class='section-title'>二、 各学科平均分走势与对比</div>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<div class='section-title'>三、 成绩梯队分布图（按人次与百分比）</div>",
    unsafe_allow_html=True,
)
...
filtered_df["score_level"] = pd.cut(
    filtered_df["score"], bins=bins, labels=labels, right=False
)
level_counts = (
    filtered_df["score_level"].value_counts().reindex(labels).fillna(0)
)
...
subject_stats = (
    filtered_df.groupby("subject")["score"]
    .agg(
...
st.markdown(
    "<div class='section-title'>五、 各学科详细数据汇总</div>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<div class='section-title'>六、 重点关注预警与针对性教研建议</div>",
    unsafe_allow_html=True,
)
...
if not low_score_students.empty:
    st.warning(
        f"⚠️ **低分预警**：本次共有 **{len(low_score_students)}** 人次成绩低于 60 分，建议安排课后专项补差。"
    )
...
for _, row in subject_stats.iterrows():
    ...
    if pass_rate < 70:
        st.error(
            f"🔴 **{subj}**（均分: {avg} | 及格率: {pass_rate}%）：及格率偏低，建议开展基础知识点专项巩固，降低作业难度梯度。"
        )
    elif avg >= 80:
        st.success(
            f"🟢 **{subj}**（均分: {avg} | 优秀率: {row['优秀率']}%）：整体表现优异，建议保持当前授课节奏，可适当增加高阶拓展题。"
        )
    else:
        st.warning(
            f"🟡 **{subj}**（均分: {avg} | 及格率: {pass_rate}%）：表现平稳，主要提升潜力集中在 60-75 分中等生群体，建议加强课堂限时训练。"
        )
